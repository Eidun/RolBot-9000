...
class Reader:
    ...
    def get_historial(self, name: str, param1: int, root):
            ...
            with open(root) as data_file:
                ...
                if(param1 == None):
                    ...
                    for i in range(100):
                        if(i <=33):
                            result[0] += str(data[name]["elemento"][i]["id"]) + "-" + data[name]["elemento"][i]["name"] + "\n"
                        if (i > 33 and i <=66):
                            result[1] += str(data[name]["elemento"][i]["id"]) + "-" + data[name]["elemento"][i]["name"] + "\n"
                        if (i > 66):
                            result[2] += str(data[name]["elemento"][i]["id"]) + "-" + data[name]["elemento"][i]["name"] + "\n"
                elif(param1 > 0 and param1 < 101 ):
                    result += str(data[name]["elemento"][param1-1]["name"]) + "-" + data[name]["elemento"][param1-1]["value"] + "\n"
    ...
    def get_arquetipos(self, name: str, root: str, param1:str):
        ...
        if(param1 == None):
            ...
            result += "BUSHIDO\n" + str(data[name]["categoria"][3]["bushido"]["name"]) + "-"+str(data[name]["categoria"][3]["bushido"]["value"])+"\nLEGADOS\n"
            result += "SHINSENGUMI\n"+"-"+str(data[name]["categoria"][3]["bushido"]["legado"]["shinsengumi"]["dato"]) + "\nESCUELAS\n"
            for k in range(10):
                result += str(data[name]["categoria"][3]["bushido"]["legado"]["shinsengumi"]["caminos"][k]["name"]) + "-" + str(data[name]["categoria"][3]["bushido"]["legado"]["shinsengumi"]["caminos"][k]["value"])+ "\n"
            result += "IKKITOUSEN\n" + "-" + str(
            ...
            result += "ZENRYOKU\n" + str(data[name]["categoria"][3]["zenryoku"]["name"]) + "-" + str(data[name]["categoria"][3]["zenryoku"]["value"]) + "\n" +str(data[name]["categoria"][3]["zenryoku"]["zenryoku"])
            result += "\nRONIN\n" + str(data[name]["categoria"][3]["ronin"]["name"]) + "-" + str(
                data[name]["categoria"][3]["ronin"]["value"]) + "\n" + str(data[name]["categoria"][3]["ronin"]["legado"])
            result += "\nLIMITES\n" + str(data[name]["categoria"][3]["limites"]) + "\nPODERES\n" + str(data[name]["categoria"][3]["poder"])
            ...
            for i in range (3):
                result +=  str(data[name]["categoria"][5]["especializacion"][i]["name"])+"-" +str(data[name]["categoria"][5]["especializacion"][i]["value"])+"\n"
            result +="TIEMPO EN ACTIVO-" + str(data[name]["categoria"][5]["tiempo"]["dato"])+"\n"
            for i in range (8):
                result +=  str(data[name]["categoria"][5]["tiempo"]["tabla"][i]["years"])+"-" +str(data[name]["categoria"][5]["tiempo"]["tabla"][i]["value"])+"\n"
            result += "VENTAJAS-" +  str(data[name]["categoria"][5]["ventajas"]["dato"])+"\n"
            for i in range (3):
                result += str(data[name]["categoria"][5]["ventajas"]["ventajas"][i]["name"]) + "-"+str(data[name]["categoria"][5]["ventajas"]["ventajas"][i]["value"]) +"\n"
    ...
    def get_experiencia(self, name: str, root: str):
        ...
        result  +="RECOMPENSAS\n"
        for i in range(5):
            result += data["experiencia"]["recompensas"][i]["cantidad"] + "-" + data["experiencia"]["recompensas"][i]["value"] +"\n"
        result += "PUNTOS INICIALES\n" + data["experiencia"]["iniciales"] + "\nCONVERSION DE PUNTOS AKUMA-5A" + data["experiencia"]["conversion"]+ "\nNOTAS\n" + data["experiencia"]["notas"] + "\nCOSTE DE LOS ANILLOS\n" + data["experiencia"]["coste-anillos"] + "\nCOSTE HABILIDAD\n" + data["experiencia"]["coste-habilidad"]
        return result
    ...
    def get_ven_des(self, name: str, root: str, param1:str):
        ...
        if(param1 == None):
            ...
            for i in range(data["ventajas"]["ventajas"].__len__()):
                result += data["ventajas"]["ventajas"][i]["name"] + " "+ data["ventajas"]["ventajas"][i]["puntos"]+"\n"
            result += "DESVENTAJA EXTRA\n" + data["ventajas"]["desventaja-extra"]["name"] + "-" +data["ventajas"]["desventaja-extra"]["value"] + "\n"
            ...
            for i in range(data["entrenamientos"].__len__()):
                if(param1 == data["entrenamientos"][i]["clan"] ):
                    for j in range(data["entrenamientos"][i]["escuelas"].__len__()):
                        result += str(data["entrenamientos"][i]["escuelas"][j]["escuela"]) + "->" + str(data["entrenamientos"][i]["escuelas"][j]["grupo"]) + "\n"
                    return result
            return "No existe esa ventaja o ya ha sido mostrada"
